...
import { getSchema, postSchema, permission } from '../../utils/index.js'
import { searchUsersAsNotFriends, updateUser } from './model.js'
import { updateUserSchema } from './schema.js'

export const usersRoute: FastifyPluginCallbackZod = (server, _options, done) => {
  server.get(
    '/',
  ...
  server.patch(
    ...
    async (req, res) => {
      ...
      if (!updatedUser)
        return res.badRequest('Rien à mettre à jour')
