	...
	class extends HTMLElement {
		...
		renderContent(): string {
			...
			<div class="grid grid-flow-col grid-rows-2 gap-2">
				...
				<div class="flex flex-row p-2 items-center justify-center text-center gap-2">${rankingPercentage}</div>
			</div>`
			return html
		}
	},
...
function getRankingPercentage(
	...
	const leagueSvg = getLeague(color)
	return leagueSvg
}

function getLeague(color: string): string {
	const leagueSvg = `
